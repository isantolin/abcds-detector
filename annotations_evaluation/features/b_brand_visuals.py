...
def detect_brand_visuals(
  ...
  """
  brand_visuals, _na = detect(config, feature_name, video_uri)
...
def detect_brand_visuals_1st_5_secs(
  ...
  """
  _na, brand_visuals_1st_5_secs = detect(config, feature_name, video_uri)
...
def detect(
  ...
  annotation_results = gcs_utils.load_annotation_blob(annotation_uri)
  ...
  # Video API: Evaluate brand_visuals_feature and
  # brand_visuals_1st_5_secs_feature 1st_5_secs
  ...
  # Evaluate brand_visuals_feature & brand_visuals_1st_5_secs in logo
  # annotations
  brand_kg_entities = get_knowledge_graph_entities(
    config, config.brand_variations
  )
  brand_kg_entities_list = []
  for value in brand_kg_entities.values():
    entity_id = value["@id"][3:] if "@id" in value else ""
    entity_name = value.get("name", "")
    entity_description = value.get("description", "")
  ...
  if "logo_recognition_annotations" in annotation_results:
    for logo_recognition_annotation in annotation_results.get(
      ...
      if len(found_entities) > 0:
        # All logo tracks where the recognized logo appears. Each
        # track corresponds
          ...
          if track.get("confidence") >= config.confidence_threshold:
            ...
            if start_time_secs <= config.early_time_seconds:
              ...
              for timestamped_object in track.get("timestamped_objects"):
                # Normalized Bounding box in a frame, where
                # the object is located.
                normalized_bounding_box = timestamped_object.get(
                  "normalized_bounding_box"
                )
                bottom_top = (normalized_bounding_box.get("bottom") or 0) - (
                  normalized_bounding_box.get("top") or 0
                )
                right_left = (normalized_bounding_box.get("right") or 0) - (
        ...
        # VideoSegment.
        # Since there is no confidence here, just check 1st 5 mins
        # feature - CHECK
        for segment in logo_recognition_annotation.get("segments"):
          start_time_secs = calculate_time_seconds(segment, "start_time_offset")
          if start_time_secs <= config.early_time_seconds:
            brand_visuals_1st_5_secs = True
  else:
    print(
      f"No Logo annotations found. Skipping {feature_name} evaluation with"
      " Video Intelligence API."
    )
