...
"""Module to detect Connect: Presence of People & (First 5 seconds)
Annotations used:
...
def detect(
  ...
  annotation_uri = (
    ...
    f"{Annotations.PEOPLE_ANNOTATIONS.value}.json"
  )
  ...
  # Video API: Evaluate presence_of_people_feature and
  # presence_of_people_1st_5_secs_feature
    ...
    for people_annotation in people_annotation_results.get(
      ...
      for track in people_annotation.get("tracks"):
        ...
        if track.get("confidence") >= config.confidence_threshold:
          ...
          # Each segment includes track.get("timestamped_objects")
          # that include
          # characteristics - -e.g.clothes, posture of the person
          # detected.
  else:
    print(
