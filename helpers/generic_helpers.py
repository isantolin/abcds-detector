    ...
    for query in queries:
      ...
      for element in response["itemListElement"]:
        ...
        if query.lower() == kg_entity_name.lower():
          kg_id = element["result"]["@id"][3:]
          kg_entities[kg_id] = element["result"]
    return kg_entities
  except Exception:
    logger.exception(
...
def trim_video(config: Configuration, video_uri: str):
  ...
  if reduced_blob is None:
    ...
    with open(FFMPEG_BUFFER, "wb") as f:
      blob = gcs_utils.get_blob(video_uri)
      if blob:
    ...
    # upload
    gcs_utils.upload_blob(reduced_uri, FFMPEG_BUFFER_REDUCED)
    ...
    for annotations_eval_feature in annotations_evaluation.get(
      ...
      if feature_found:
        ...
        print(
          ...
          f" {annotations_eval_feature.get('id')} not found. "
  ...
  if llms_evaluation:
    for llms_eval_feature in llms_evaluation.get("evaluated_features"):
      ...
      if feature_found:
        ...
        print(
          f"LLMs evaluation: Feature {llms_eval_feature.get('id')} "
          "not found. Skipping from storing it in BQ. \n"
        )
  else:
...
def store_in_bq(
  ...
  if len(assessment_bq) > 0:
    ...
    job.result()
    print(f"Rows inserted in {full_table_name} successfully!\n")
  else:
    print(
